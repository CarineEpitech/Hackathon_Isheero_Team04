...
Le Score de Tension Territorial compare l'activité des 14 derniers jours à la baseline des 90 jours précédents pour chaque département. Le calcul pondère le ton moyen (35 %), les codes sécuritaires CAMEO (40 %), le volume d'événements (15 %) et la diversité des sources (10 %).
...
Ce projet a utilisé **Claude Sonnet 4.6 (Anthropic)** comme assistant au développement : architecture du pipeline de données, conception de l'algorithme STT, debugging, rédaction de scripts Python et du rapport final.

Les scores AvgTone, GoldsteinScale et codes CAMEO présents dans les données brutes sont produits par les modèles NLP internes au projet GDELT — ils sont utilisés comme features, non recalculés par l'équipe.

Aucun modèle propriétaire n'a été entraîné. Aucune donnée personnelle n'a été transmise à un LLM externe.
