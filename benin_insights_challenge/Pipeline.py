...
def run_pipeline(input_path=None, output_dir=None):
    ...
    if input_path is None:
        input_path = ROOT_DIR / "data/raw/benin_raw.csv"# Chemin vers le CSV brut extrait de BigQuery
    if output_dir is None:
        output_dir = ROOT_DIR / "data/processed"# Dossier de sortie pour les fichiers nettoyés

    print("=" * 55)
    print("PIPELINE GDELT — BENIN INSIGHTS CHALLENGE")
    print("=" * 55)
    ...
    assert len(df) > 100, f" Trop peu d'événements : {len(df)}"# Vérifier qu'on a suffisamment de données

    assert df['GLOBALEVENTID'].duplicated().sum() == 0, " Doublons détectés !"# Vérifier l'absence totale de doublons

    assert df['SQLDATE'].isna().sum() == 0, " Dates manquantes !"# Vérifier que toutes les dates sont valides

    assert df['SQLDATE'].min().year >= 2025, " Données avant 2025 détectées !"# Vérifier que les données respectent le filtre temporel

    if 'ActionGeo_Lat' in df.columns:
        lat_valides = df['ActionGeo_Lat'].dropna()
        if len(lat_valides) > 0:
            assert lat_valides.between(-90, 90).all(), " Latitudes hors limites !"# Vérifier que les latitudes sont dans l'intervalle valide
    ...
    assert df_clean.shape[1] < df.shape[1], (
        f"Erreur : benin_clean ({df_clean.shape[1]} colonnes) devrait avoir "f"moins de colonnes que benin_enrichi ({df.shape[1]} colonnes).")
...
if __name__ == "__main__":
    # Ce bloc s'exécute uniquement quand on lance "python pipeline.py"# Il ne s'exécute PAS si le fichier est importé depuis un autre script
    run_pipeline()